...
class PyTorchBasics:
    ...
    def make_it_pytorch_3(x: torch.Tensor) -> torch.Tensor:
        ...
        for i in x.flatten():
            if i not in y:
                y.append(i)
        return torch.as_tensor(sorted=True)
        --------
    ...
    def make_it_pytorch_10(x: torch.Tensor, c: torch.Tensor) -> torch.Tensor:
        ...
        """
        return torch.where(x >= c, x, torch.tensor(0.0, device=x.device, dtype=x.dtype))
        """
        raise NotImplementedError
        """
    ...
    def make_it_pytorch_11(x: torch.Tensor, c: torch.Tensor) -> torch.Tensor:
        ...
        """
        return torch.nonzero(x < c, as_tuple=True)
        """
        raise NotImplementedError
        """
    ...
    def make_it_pytorch_extra_2(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        """
        return ((x.unsqueeze(1) - y.unsqueeze(0)).abs() < 1e-3).any(dim=1).sum()
        """
        raise NotImplementedError
        """
